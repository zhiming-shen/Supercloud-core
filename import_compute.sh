...
IP_SUFFIX=$IPSUFFIX
STACK_SUFFIX=$((IP_SUFFIX+50))

set -e
#MAKE SURE to unmount if script does not run to completion
mkdir -p $TEMP/snapshots
cd $TEMP/snapshots
wget http://fireless.cs.cornell.edu/%7Ezshen/compute.xva.tgz
tar xzf compute.xva.tgz
cd ..

xe vm-import filename=snapshots/compute.xva
xe vm-param-set name-label=$HOSTNAME uuid=`xe vm-list name-label=compute-101 --minimal`
...
sed -c -i "s/compute-101/$HOSTNAME/" hosts
