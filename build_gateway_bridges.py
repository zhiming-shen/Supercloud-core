...
for index, bridge in enumerate(bridges):
    ...
    os.system("ovs-vsctl add-port %s patch_gateway_%s -- set Interface patch_gateway_%s type=patch options:peer=patch_%s" % (bridge_name, bridge_name, bridge_name, tunnel_name))
    os.system("ovs-vsctl add-port %s patch_%s -- set Interface patch_%s type=patch options:peer=patch_gateway_%s" % (tunnel_name, tunnel_name, tunnel_name, bridge_name))
